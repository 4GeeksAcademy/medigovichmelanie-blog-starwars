...
export const PlanetCard = ({ planet }) => {
    ...
    const [isFavorite, setIsFavorite] = useState(store.favorites.includes(planet.name));
    ...
    const handleClick = () => {
        dispatch({ type: "handle_favorites", payload: planet.name });
        setIsFavorite(!isFavorite);
    }

    useEffect(() => {
        getDetails();
        setIsFavorite(store.favorites.includes(planet.name));
    }, [store.favorites]);
        ...
        <div className="col-md-3 col-lg-2 col-sm-6 col-12 mb-4 px-2">
            <div className="card h-100 shadow-sm border-0 hover-effect">
                ...
                <div className="card-body d-flex flex-column p-3">
                    <h5 className="card-title text-truncate mb-3">{planet?.name}</h5>
                    <p className="card-text mb-1"><strong>Population:</strong> {detalle.population}</p>
                    <p className="card-text mb-1"><strong>Terrain:</strong> {detalle.terrain}</p>
                    <p className="card-text mb-3"><strong>Gravity:</strong> {detalle.gravity}</p>
