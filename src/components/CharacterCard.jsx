...
export const CharacterCard = ({ character }) => {
    ...
    const [isFavorite, setIsFavorite] = useState(store.favorites.includes(character.name));
    ...
    const handleClick = () => {
        dispatch({ type: "handle_favorites", payload: character.name });
        ...
        <div className="col-md-3 col-lg-2 col-sm-6 col-12 mb-4 px-2">
            <div className="card h-100 shadow-sm border-0 hover-effect">
                ...
                <div className="card-body d-flex flex-column p-3">
                    <h5 className="card-title text-truncate mb-3">{character?.name}</h5>
                    <p className="card-text mb-1"><strong>Gender:</strong> {detalle.gender}</p>
                    <p className="card-text mb-1"><strong>Hair Color:</strong> {detalle.hair_color}</p>
                    <p className="card-text mb-3"><strong>Eye Color:</strong> {detalle.eye_color}</p>
