...
export const VehicleCard = ({ vehicle }) => {
    ...
    const [isFavorite, setIsFavorite] = useState(store.favorites.includes(vehicle?.properties.name));
    ...
    const handleClick = () => {
        dispatch({ type: "handle_favorites", payload: vehicle?.properties.name });
        setIsFavorite(!isFavorite);
    }

    useEffect(() => {
        setIsFavorite(store.favorites.includes(vehicle?.properties.name));
    }, [store.favorites]);
        ...
        <div className="col-md-3 col-lg-2 col-sm-6 col-12 mb-4 px-2">
            <div className="card h-100 shadow-sm border-0 hover-effect">
                ...
                <div className="card-body d-flex flex-column p-3">
                    <h5 className="card-title text-truncate mb-3">{vehicle?.properties.name}</h5>
                    <p className="card-text mb-1"><strong>Model:</strong> {vehicle?.properties.model}</p>
                    <p className="card-text mb-1"><strong>Passengers:</strong> {vehicle?.properties.passengers}</p>
                    <p className="card-text mb-3"><strong>Vehicle Class:</strong> {vehicle?.properties.vehicle_class}</p>
